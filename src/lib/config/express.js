...
module.exports = (app, config) => {
  ...
  // Attach to the log4js logger (instantiated by <app_root>/src/lib/config/logging.js)
  let logger = logging.getLogger('backup');
  ...
  nunjucks.configure(path.join(config.app.root, 'src', 'app', 'views'), {
    autoescape: true,
    express: app
  });
  ...
  //
  let controllers = glob.sync(path.join(config.app.root, 'src', 'app', 'controllers','**','*.js'));
